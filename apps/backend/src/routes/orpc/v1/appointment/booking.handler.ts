...
export const createBookingHandler = base
	...
	.handler(async ({ input, context: { var: { drizzle: db, auth, emailQueue }, req } }) => {
		const {
			organizationId,
			serviceId,
			employeeId,
		...
		let user = await db.query.user.findFirst({ where: (userTable, { eq }) => eq(userTable.email, contact.email) });
		...
		let customer = await db.query.customer.findFirst({
			where: (customerTable, { and, eq }) => and(
		...
		const employeeUser = await db.query.user.findFirst({
			where: (userTable, { eq }) => eq(userTable.id, bestEmployee.member.userId),
		});
