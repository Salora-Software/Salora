...
export const getAppointmentsHandler = base
	...
	.handler(async ({
		...
		context: { var: { drizzle: db, auth }, req },
	}) => {
		...
		const filteredAppointments = appointments.filter((appointment) => {
			if (targetCustomerId && appointment.booking?.customerId === targetCustomerId) return true;
		...
		return filteredAppointments.map((appointment) => {
			...
			if (
				...
				return {
					...appointment,
					booking: appointment.booking ? { ...appointment.booking, status: 'COMPLETED' } : null,
				};
			}
