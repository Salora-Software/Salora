  ...
  app.get("/docs/spec.json", async (c) => {
    if (c.var.env.NODE_ENV !== "development") {
      throw new ORPCError("FORBIDDEN", {
        message:
          "Access to API specification is restricted in production environment",
      });
    }
    const spec = await openAPIGenerator.generate(router, {
      info: {
        ...
        version: "1.0.0",
        description:
          "A modern, type-safe API built with oRPC and Hono. Explore our endpoints for authentication, data management, and more.",
        contact: {
          name: "Salora Team",
