...
<div class="flex rounded-md shadow-sm shadow-black/[.04]">
	...
	>
		<div class="inline-flex items-center gap-1" aria-hidden="true">
			<span class="flex h-[16px] w-5 items-center overflow-hidden rounded-sm">
				{#if selectedCountry}
					<span class="flag flag-{selectedCountry.toLowerCase()} !h-[13px] !w-5" aria-hidden="true"
					></span>
				{:else}
	...
	<TelInput
		...
		class={cn(
			'border-input bg-background ring-offset-background placeholder:text-muted-foreground flex h-10 w-full rounded-l-none rounded-r-lg border px-3 py-2 text-base file:border-0 file:bg-transparent file:text-sm file:font-medium focus:border-black focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
			className
		)}
		bind:value
	/>
