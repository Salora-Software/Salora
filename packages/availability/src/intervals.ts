...
export const getIntervalsForDate = (shifts: WeekShift[], date: DateTime, timeZone: string) => {
	const targetWeekday = date.weekday === 7 ? 0 : date.weekday;

	return shifts
		.filter((s) => s.dayOfWeek === targetWeekday)
		.map((s) => {
			const sStart = DateTime.fromJSDate(new Date(s.startTimeUtc), { zone: 'UTC' }).setZone(timeZone);
