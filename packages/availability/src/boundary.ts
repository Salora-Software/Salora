...
export const getDaySpanForDateTime = (
  date: DateTime,
...
export const getDaySpanForJsDate = (
  date: Date,
  timeZone: string = DEFAULT_ZONE,
): DaySpan => {
...
export const getRangeSpanForInterval = (
  range: Interval,
  timeZone: string = DEFAULT_ZONE,
): DaySpan => {
  const utcStart = range
    .start!.setZone(DEFAULT_ZONE, { keepLocalTime: true })
    .startOf("day");
  const utcEnd = range
    .end!.setZone(DEFAULT_ZONE, { keepLocalTime: true })
    .endOf("day");
  const localStart = range
    .start!.setZone(timeZone, { keepLocalTime: true })
    .startOf("day");
  const localEnd = range
    .end!.setZone(timeZone, { keepLocalTime: true })
    .endOf("day");
