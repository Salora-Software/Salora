export { default as AppointmentEmail } from "./appointment";
export {
  AppointmentEmailSchema,
  type AppointmentEmailVariables,
} from "./appointment.config";

export { default as ForgotPasswordEmail } from "./forgot-password";
// export { ForgotPasswordEmailSchema, type ForgotPasswordEmailVariables } from './forgot-password.config';

export { default as AppointmentCancelledEmail } from "./appointment-cancelled";
// export { AppointmentCancelledEmailSchema, type AppointmentCancelledEmailVariables } from './appointment-cancelled.config';

export { default as AppointmentEmployeeEmail } from "./appointment-employee";
export { default as AppointmentCancelledEmployeeEmail } from "./appointment-cancelled-employee";
export { default as AppointmentPendingEmail } from "./appointment-pending";
export { default as AppointmentPendingEmployeeEmail } from "./appointment-pending-employee";
