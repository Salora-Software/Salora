	...
	export const sheetVariants = tv({
		...
		variants: {
			side: {
				...
				// 'data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right inset-y-0 right-0 h-[calc(100%-1.5rem)] m-3 w-3/4 border-l sm:max-w-sm !max-w-[calc(100%-1.5rem)] rounded-lg'
			}
		},
		defaultVariants: {
			side: 'right'
...
<SheetPrimitive.Portal {...portalProps}>
	...
	<SheetPrimitive.Content bind:ref class={cn(sheetVariants({ side }), className)} {...restProps}>
		<div class="grid h-full max-h-[100vh] grid-rows-[1fr_auto]">
			{@render children?.()}
		</div>
