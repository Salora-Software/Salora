    ...
    async ({
      ...
      const organization = await db.query.organization.findFirst({
        where: (organizationTable, { eq }) =>
          eq(organizationTable.id, branchId),
      });
      ...
      const calendarItem = await db.query.calendarItem.findFirst({
        where: (calendarItemTable, { eq }) =>
          eq(calendarItemTable.id, appointmentId),
        with: {
          booking: {
            with: {
              customer: true,
      ...
      const isOwnerById = Boolean(
        customer?.id && booking.customerId === customer.id,
      );
      const isOwnerByEmail = Boolean(
        session.user?.email && booking.customer?.email === session.user.email,
      );
