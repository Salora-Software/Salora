...
2. Replace the text with Paraglide message calls: `m.key_name({ var: 'value' })`. Note that Paraglide flattens nested JSON keys into snake_case or camelCase function names (e.g., `dashboard.hello` becomes `m.dashboard_hello()`).
...
5. Replace the hardcoded text in the Svelte file with `{m.your_flattened_key()}`.