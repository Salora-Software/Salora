...
export const getAvailabilityHandler = base
	...
	.handler(async ({ input: { branchId, serviceId, date }, context: { var: { drizzle: db } } }) => {
		const initialSpan = getDaySpanForDateTime(date);

		const { organization, employees, timeZone, engine } = await createAppointmentContext(
			db,
			branchId,
			serviceId,
			initialSpan.utcSpan,
		...
		const allBookings = employees.flatMap((employee) => employee.member.calendarItems);
